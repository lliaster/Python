...
from motor.motor_asyncio import AsyncIOMotorClient       #pymongo同步驱动
...
class CarSpider:
    ...
    def __init__(self):
        ...
        self.user_agent = UserAgent()  #不可以在这里实例化  因为构造方法只可以用一次
    ...
    async def get_car_id(self, page, session):
        headers = {'User-Agent': self.user_agent.random}  #参考26行 所以要放到外面去
