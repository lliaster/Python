...
class MgTV:
    def __init__(self):
        ...
        self.redis_client = redis.Redis(host='192.168.1.14', port=6379, db=0)
    ...
    @staticmethod# 静态方法 因为它不需要实例化对象就可以直接调用
    def get_md5(response):
        # md5方法接收的参数必须是字节, md5方法中的hexdigest返回的是编码之后的md5结果值
        md5_hash = hashlib.md5(str(response).encode('utf-8')).hexdigest() #MD5所接收的参数必须是字节, md5方法中的hexdigest返回的是编码之后的md5结果值
        return md5_hash
    ...
    def save_movie_info(self, item):
        ...
        flag = self.redis_client.sadd('movie:filter', md5_hash)  # 'movie:filter' 会生成一个类似文件夹的数据结构
